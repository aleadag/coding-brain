...
`INCOMPLETE` with `permission evaluation timed out` means an `Observed` or `Evaluating` permission request became stale without a terminal event. It remains in Needs Attention, but it is a projection only: Coding Brain does not append an `Incomplete` row to `activity.jsonl`, and elapsed time does not prove that the hook died. It is distinct from an interrupted tool and does not prove that a response was delivered or a command executed.

Before projecting activity, Brain refresh attempts bounded recovery of pending permission transactions. A valid interrupted transaction is completed to durable destination evidence; a proposal that lacks executable authority becomes a terminal error rather than an approval. Active, invalid, or otherwise unresolved journals remain retained instead of being treated as recovered.
...
Diagnostics is a metadata-only viewer for safe categories covering hook and correlation faults, rejected or uncertain session actions and recovery, and activity-store integrity. It is not ordinary command output and never stores captured terminal content or manual text. Use `j`/`k` or the arrow keys to select recent diagnostic events. Review and archive keys change only the Diagnostics projection; they do not modify the raw audit rows in `$XDG_STATE_HOME/coding-brain/activity.jsonl`. Diagnostics displays Store integrity counters for malformed rows, duplicate terminal states, truncated tails, and discarded tail bytes. Event rows identify the provider, project, and tool, while Evidence shows the selected event's metadata and reason. Visible control characters are escaped before display. At 120 columns and wider, the event list and Evidence appear side by side; in narrower terminals they stack, and PageUp/PageDown scroll Evidence when its `↑ more` or `↓ more` title indicators appear.
...
- `doctor` checks the executable, hook definitions, affected provider compatibility, trust visibility, project identity, permission transaction recovery, lifecycle state, outcome telemetry, endpoint privacy, transcript discovery, and terminal integration.
- `Permission transaction recovery` is omitted for a clean store. Completed recovery is Pass; active locked transactions are Advisory; invalid, over-budget, unresolved, removal-sync-uncertain, or recovery-error conditions are Fail. An over-budget failure includes `over_budget_store` with one fixed source (`journal_count`, `journal_bytes`, `decisions.jsonl`, or `activity.jsonl`) and the enforced numeric `over_budget_limit`; it never includes stored content or a caller-controlled path.
