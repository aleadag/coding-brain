...
- `doctor` checks the executable, hook definitions, affected provider compatibility, authoritative Codex hook trust, project identity, SQLite schema and migration status, WAL size, endpoint privacy, transcript discovery, and terminal integration.
...
- `doctor` emits one setup row for Codex, Claude, and Antigravity, plus separate compatibility, Codex hook trust, Agent Deck navigation, Claude native attach, guarded semantic input, and focus-only fallback rows. Each non-current file candidate appears in inspection order with its path, scope, ownership, state, and a bounded reason when applicable, even when another file makes the aggregate provider setup current; the list is empty when no candidate path can be resolved, such as when `HOME` is unavailable. `doctor --json` returns the same records under `evidence.provider_files`. The Codex trust row asks the bounded `hooks/list` app-server method for effective hook status: enabled cbrain hooks reported as trusted or managed pass, modified or untrusted hooks advise review through `/hooks`, and unavailable or malformed trust results fail closed to an advisory. With current managed Antigravity hooks, exact `agy` 1.1.5 produces an `Antigravity hook contract` advisory because that version may retain the native prompt after a valid hook decision. Other versions remain unverified. An unselected absent provider is skipped, while a selected provider with a missing executable is advisory. For invalid or stale declaratively managed hooks, rebuild Home Manager and restart the affected provider; for imperatively managed providers, run the `cbrain init <provider>` repair command shown in the provider row.
