...
Outside Live action mode, press `a` to review the selected NEW item and `A` to confirm all visible NEW items. Attention, Review, and Diagnostics also use `d`/`D` to archive reviewed items and `u` to undo the latest archive. Recent uses `a`/`A` to mark items seen and cannot be archived; Review keeps `s` as review-and-next. These actions change only the active view's lifecycle state, not its underlying evidence or Scorecard results. See [Review lifecycle state](configuration.md#review-lifecycle-state) for persistence, first-run cleanup, and reset behavior.
